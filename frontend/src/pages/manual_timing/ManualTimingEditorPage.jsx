...
function buildManualTimingSceneWordText(words = []) {
  if (!Array.isArray(words) || !words.length) return "";
  return words
    .map((word) => String(word?.word || word?.text || word?.raw || "").trim())
    .filter(Boolean)
    .join(" ")
    .replace(/\s+/g, " ")
    .trim();
...
export default function ManualTimingEditorPage() {
  ...
  const [isAdvancedSceneMarkupOpen, setIsAdvancedSceneMarkupOpen] = useState(false);
  ...
  const [sceneAsrInspectorMode, setSceneAsrInspectorMode] = useState("scene_words");
  ...
  const manualTimingPausedQueueRef = useRef(null);
  ...
  const advancedSceneSummary = useMemo(() => ({
    scenesCount: scenes.length,
    vocalCount: scenes.filter((scene) => Boolean(scene?.contains_vocal)).length,
    i2vCount: scenes.filter((scene) => String(scene?.route || "") === "i2v").length,
    ia2vCount: scenes.filter((scene) => String(scene?.route || "") === "ia2v").length,
    shortCount: scenes.filter((scene) => Number(scene?.duration_sec || 0) > 0 && Number(scene?.duration_sec || 0) < 2.2).length,
    warningsCount: warnings.length,
  }), [scenes, warnings.length]);
  ...
  function getCurrentQueueTimelineTime() {
    const session = manualTimingQueuePlaybackRef.current;
    const audioEl = audioRef.current;

    if (!session) return roundTimingSec(currentTimeRef.current || currentTime || 0);

    const scene = session.scenes?.[session.index];
    if (!scene) return roundTimingSec(currentTimeRef.current || currentTime || 0);

    const timelineStart = roundTimingSec(Number(scene.start_sec || 0));
    const timelineEnd = roundTimingSec(Number(scene.end_sec || timelineStart));

    if (isManualTimingSilenceScene(scene)) {
      return roundTimingSec(clampTime(currentTimeRef.current || timelineStart, timelineEnd));
    }

    const sourceStart = Number(scene.source_start_sec ?? scene.sourceStartSec);
    const sourceEnd = Number(scene.source_end_sec ?? scene.sourceEndSec);

    if (audioEl && Number.isFinite(sourceStart) && Number.isFinite(sourceEnd) && sourceEnd > sourceStart) {
      const sourceTime = roundTimingSec(Math.max(sourceStart, Math.min(sourceEnd, Number(audioEl.currentTime || sourceStart))));
      return roundTimingSec(Math.max(timelineStart, Math.min(timelineEnd, timelineStart + (sourceTime - sourceStart))));
    }

    return roundTimingSec(currentTimeRef.current || timelineStart);
  }
  ...
  function pauseManualTimingQueuePlayback() {
    ...
    manualTimingPausedQueueRef.current = {
      mode: session.mode || "scene",
      timelineTimeSec: pausedTimelineTime,
      sceneIndex: Number(session.index || 0),
      sceneId: session.scenes?.[session.index]?.scene_id || "",
    ...
    setDisplayTime(pausedTimelineTime);
    setPlayingState(false);
  }

  function resumeManualTimingPausedQueue() {
    const paused = manualTimingPausedQueueRef.current;
    if (!paused) return false;

    const safeScenes = Array.isArray(paused.scenes) && paused.scenes.length
      ? paused.scenes
      : (paused.mode === "all" ? buildManualTimingPlaybackQueue(scenes, scenes) : []);
    ...
    const position = findManualTimingTimelinePosition(paused.timelineTimeSec, safeScenes);
    const resumeIndex = position?.index >= 0
      ? position.index
      : Math.max(0, Number(paused.sceneIndex || 0));

    const session = {
      mode: paused.mode || "scene",
      scenes: safeScenes,
      index: resumeIndex,
      rangeEndSec: null,
      startedAtMs: 0,
  ...
  function playManualTimingQueueItem(session, index, startTimelineOverrideSec = null) {
    ...
    const effectiveTimelineStart = Number.isFinite(Number(startTimelineOverrideSec))
      ? roundTimingSec(Math.max(timelineStart, Math.min(timelineEnd, Number(startTimelineOverrideSec))))
      : timelineStart;

    if (isManualTimingSilenceScene(scene)) {
      const silenceStart = effectiveTimelineStart;
      const silenceEnd = timelineEnd;

      try { audioEl.pause(); } catch {}

      session.startedAtMs = performance.now();
      session.silenceStartTimelineSec = silenceStart;
      session.silenceEndTimelineSec = silenceEnd;
      ...
      const tick = () => {
        ...
        const nextTimeline = roundTimingSec(Math.min(silenceEnd, silenceStart + elapsed));
    ...
    const effectiveSourceStart = roundTimingSec(
      Math.max(sourceStart, Math.min(sourceEnd, sourceStart + (effectiveTimelineStart - timelineStart)))
    );

    try {
      audioEl.pause();
      audioEl.currentTime = effectiveSourceStart;
    } catch {}

    setDisplayTime(effectiveTimelineStart);
    setPlayingState(true);
  ...
  const onPlayPause = () => {
    ...
    if (manualTimingQueuePlaybackRef.current || !audioEl.paused || isPlayingRef.current) {
      pauseManualTimingQueuePlayback();
      return;
    }
  ...
  const onCreateQuickDirectorBoard = async () => {
    ...
    const quickScenes = quickSourceScenes.map((scene, index) => {
      ...
      return {
        ...
        end_sec: endSec,
        duration_sec: durationSec,
        target_t0: Number(scene?.target_t0 ?? startSec),
        target_t1: Number(scene?.target_t1 ?? endSec),
        source_phrase_ids: normalizedPhraseIds,
  ...
  const inferVideoMatchSceneMarkersFromNote = (note = "", scene = {}) => {
    ...
    const priorityWeight = { low: 0, normal: 1, high: 2, critical: 3 };
    const chooseHigherPriority = (currentPriority = "normal", nextPriority = "normal") => {
      const currentWeight = priorityWeight[currentPriority] ?? priorityWeight.normal;
      const nextWeight = priorityWeight[nextPriority] ?? priorityWeight.normal;
      return nextWeight >= currentWeight ? nextPriority : currentPriority;
    };
    const applyMarker = ({ label = "", color = "", role = "", intent = "", priority = "" }) => {
      const currentPriority = labelPriority[result.user_scene_label] || 0;
      const nextPriority = labelPriority[label] || 0;
      if (nextPriority >= currentPriority) {
        result.user_scene_label = label;
        result.user_scene_color = color || result.user_scene_color;
        result.video_match_role = role || result.video_match_role;
        result.scene_render_intent = intent || result.scene_render_intent;
      }
      if (priority) {
        result.user_scene_priority = chooseHigherPriority(result.user_scene_priority, priority);
      }
    };

    if (hasAny(["липсинк", "lip sync", "lipsync", "поёт", "поет", "говорит в кадр", "крупный план рот"])) {
      applyMarker({ label: "LIP-SYNC", color: "#A855F7", role: "performance", intent: "performance", priority: "high" });
    ...
    if (hasAny(["экшен", "боевик", "драка", "погоня", "action"])) {
      applyMarker({ label: "ACTION", color: "#EF4444", role: "action", intent: "action", priority: "high" });
      ["action", "movement"].forEach((tag) => tags.add(tag));
    }
    if (hasAny(["деталь", "предмет", "перебивка", "b-roll", "broll"])) {
      applyMarker({ label: "B-ROLL", color: "#22C55E", role: "detail", intent: "b_roll", priority: "normal" });
      ["b_roll", "detail"].forEach((tag) => tags.add(tag));
    }
    if (hasAny(["финал", "концовка", "closing", "outro"])) {
      applyMarker({ label: "CLOSING", color: "#F59E0B", role: "closing", intent: "ending", priority: "normal" });
  ...
  const buildManualTimingVideoMatchSeedJson = (sourceProject = {}) => {
    ...
    return {
      ...
      chatgpt_composition_interview: {
        required: true,
        must_ask_before_codex_job: true,
        main_question_ru: "На какую композицию делаем монтаж?",
        explain_to_user_ru: "Перед отправкой в Codex нужно выбрать режиссёрскую композицию: что главное в монтаже, какие сцены оставить под lip-sync, где пики, какой финал и стиль.",
        questions_ru: [
        ...
        after_user_answers_chatgpt_must_create: [
          "composition_brief",
          "scene_by_scene_emotional_guide",
          "reserved_user_override_scenes",
          "codex_video_match_job_prompt",
        ],
      },
      composition_brief: {
        ...
        composition_type: "",
        main_visual_priority: "",
        emotional_arc_ru: "",
        visual_arc_ru: "",
        reserved_override_policy_ru: "",
        avoid_ru: "",
        notes_ru: "",
      },
      video_match_policy: {
        requires_user_composition_before_codex: true,
        composition_must_define: [
          "main_visual_priority",
          "emotional_arc",
          "scene_by_scene_intent",
          "reserved_override_scenes",
    ...
    <div className={`manualTimingPage pageCard ${modeConfig.className}`} data-build="manual-timing-stable-v18" data-story-bible-build="manual-timing-story-bible-v21" data-json-help-build="manual-timing-json-help-v22">
              ...
              <div className="manualTimingSelectedPhraseRows">
                  ...
                  selectedSceneWordText
                    ? <div className="manualTimingSelectedPhraseRow">
                      <div className="manualTimingSelectedPhraseOriginal">
                        ...
                        <span className="manualTimingPhraseModeHint">Оригинал ASR по word timestamps. Это не перевод.</span>
                ...
                >
                  <div className="manualTimingSelectedPhraseOriginal">
                    ...
                    <span className="manualTimingPhraseModeHint">ASR-фразы могут быть разбиты внутри одной сцены. Это не разрез сцены.</span>
          ...
          <div className="manualTimingTrackToolbox">
            <div className="manualTimingPlayStack">
              ...
              <button
                ...
                onClick={() => {
                  const paused = manualTimingPausedQueueRef.current;
                  if (manualTimingQueuePlaybackRef.current && manualTimingPlaybackMode === "full_timeline") {
                    pauseManualTimingQueuePlayback();
                    return;
                  }
                  if (paused?.mode === "all") {
                    resumeManualTimingPausedQueue();
                    return;
                  }
              ...
              >
                {isPlaying && manualTimingPlaybackMode === "full_timeline" ? "■ всё" : "▶ всё"}
              </button>
            </div>
      ...
      <section className="manualTimingAdvancedScenePanel">
        ...
        <div className="manualTimingAdvancedSceneSummary">
          ...
          {advancedSceneSummary.warningsCount > 0 ? <b>Есть предупреждения: {advancedSceneSummary.warningsCount}</b> : null}
        </div>
        <button className="clipSB_btn clipSB_btnSecondary manualTimingAdvancedSceneToggle" type="button" onClick={() => setIsAdvancedSceneMarkupOpen((prev) => !prev)}>{isAdvancedSceneMarkupOpen ? "Скрыть разметку сцен" : "Развернуть разметку сцен"}</button>
        {isAdvancedSceneMarkupOpen ? <div className="manualTimingAdvancedSceneBody"><section className="manualTimingRows manualTimingAdvancedSceneList">
        {scenes.map((scene, idx) => {
