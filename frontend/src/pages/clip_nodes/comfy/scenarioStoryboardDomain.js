...
const LEGACY_PROMPT_GLOBAL_ARTIFACT_PATTERNS = [
...
const LEGACY_DOMESTIC_POSITIVE_VENUE_PATTERNS = [
  /\bnightclub\b/ig,
  /\bclub\b/ig,
  /\bdance floor\b/ig,
  /\bbar\b/ig,
...
function sanitizeLegacyPositivePromptArtifacts(
  value = "",
  { injectControlledMotion = false, domestic = false } = {}
) {
  let text = normalizeText(value);
  if (!text) return "";
  LEGACY_PROMPT_GLOBAL_ARTIFACT_PATTERNS.forEach((pattern) => {
    text = text.replace(pattern, " ");
  });
  if (domestic) {
    LEGACY_DOMESTIC_POSITIVE_VENUE_PATTERNS.forEach((pattern) => {
      text = text.replace(pattern, " ");
    });
  }
...
function sanitizeLegacyNegativePromptArtifacts(value = "") {
  let text = normalizeText(value);
  if (!text) return "";
  LEGACY_PROMPT_GLOBAL_ARTIFACT_PATTERNS.forEach((pattern) => {
    text = text.replace(pattern, " ");
...
function sanitizeVisiblePromptText(value = "") {
  ...
  text = text
    ...
    .replace(/\s+/g, " ")
    .trim();
  text = sanitizeLegacyPositivePromptArtifacts(text);
  if (!text) return "";
...
export function normalizeScenarioScene(scene = {}, index = 0, scenarioPackage = null) {
  ...
  const sanitizedPositiveForScene = sanitizeLegacyPositivePromptArtifacts(canonicalPositiveForScene, {
    injectControlledMotion: shouldInjectDomesticControlledMotion,
    domestic: isDomesticScene(source),
  });
  const sanitizedNegativeForScene = sanitizeLegacyNegativePromptArtifacts(canonicalNegativeForScene);
  ...
  const normalizedScene = {
    ...
    finalVideoPrompt: {
      ...
      negativePrompt: sanitizedNegativeForScene,
    ...
    source_image_refs: Array.isArray(source.source_image_refs) ? source.source_image_refs : (Array.isArray(source.sourceImageRefs) ? source.sourceImageRefs : []),
    sourceImageRefs: Array.isArray(source.sourceImageRefs) ? source.sourceImageRefs : (Array.isArray(source.source_image_refs) ? source.source_image_refs : []),
    linked_assets: (source.linked_assets && typeof source.linked_assets === "object") ? source.linked_assets : {},
    linkedAssets: (source.linked_assets && typeof source.linked_assets === "object") ? source.linked_assets : {},
    ...
    primary_role: source.primaryRole ?? source.primary_role,
    visualFocusRole: source.visualFocusRole ?? source.visual_focus_role,
    visual_focus_role: source.visualFocusRole ?? source.visual_focus_role,
    ...
    lipSyncShotVariant: normalizeText(source.lipSyncShotVariant ?? source.lip_sync_shot_variant),
    performancePose: normalizeText(source.performancePose ?? source.performance_pose),
    cameraAngle: normalizeText(source.cameraAngle ?? source.camera_angle),
    gesture: normalizeText(source.gesture),
    locationZone: normalizeText(source.locationZone ?? source.location_zone),
    mouthReadability: normalizeText(source.mouthReadability ?? source.mouth_readability),
    whyThisLipSyncShotIsDifferent: normalizeText(source.whyThisLipSyncShotIsDifferent ?? source.why_this_lip_sync_shot_is_different),
    startsFromPreviousLogic: normalizeText(source.startsFromPreviousLogic ?? source.starts_from_previous_logic),
    endsWithState: normalizeText(source.endsWithState ?? source.ends_with_state),
    continuityWithNext: normalizeText(source.continuityWithNext ?? source.continuity_with_next),
    potentialContradiction: normalizeText(source.potentialContradiction ?? source.potential_contradiction),
    fixIfNeeded: normalizeText(source.fixIfNeeded ?? source.fix_if_needed),
    identityLockApplied: Boolean(source.identityLockApplied ?? source.identity_lock_applied),
    bodyLockApplied: Boolean(source.bodyLockApplied ?? source.body_lock_applied),
    wardrobeLockApplied: Boolean(source.wardrobeLockApplied ?? source.wardrobe_lock_applied),
    confirmedHeroLookReferenceUsed: Boolean(source.confirmedHeroLookReferenceUsed ?? source.confirmed_hero_look_reference_used),
    lipSyncShotVariantRepeatedWithPrevious: Boolean(source.lipSyncShotVariantRepeatedWithPrevious ?? source.lip_sync_shot_variant_repeated_with_previous),
    continuityWarning: normalizeText(source.continuityWarning ?? source.continuity_warning),
    continuityFixApplied: Boolean(source.continuityFixApplied ?? source.continuity_fix_applied),
    clipDecisionReason,
  ...
  if (isLipSyncRoute) {
    ...
    normalizedScene.videoPromptEn = sanitizeVisiblePromptText(
      sanitizeLegacyPositivePromptArtifacts(
        `${normalizedScene.videoPromptEn}. ${CONTROLLED_MOTION_SAFETY_BLOCK}. ${orbitCanonSuffix}`,
        { injectControlledMotion: true, domestic: true }
      )
    );
    normalizedScene.imagePromptEn = sanitizeVisiblePromptText(
      sanitizeLegacyPositivePromptArtifacts(normalizedScene.imagePromptEn, { domestic: true })
    );
  } else {
  ...
  if (isDomesticScene(normalizedScene)) {
    normalizedScene.startFramePromptRu = sanitizeVisiblePromptText(
      sanitizeLegacyPositivePromptArtifacts(normalizedScene.startFramePromptRu || DOMESTIC_FRAME_FALLBACK, { domestic: true })
    );
    normalizedScene.startFramePromptEn = sanitizeVisiblePromptText(
      sanitizeLegacyPositivePromptArtifacts(normalizedScene.startFramePromptEn || DOMESTIC_FRAME_FALLBACK, { domestic: true })
    );
    normalizedScene.endFramePromptRu = sanitizeVisiblePromptText(
      sanitizeLegacyPositivePromptArtifacts(normalizedScene.endFramePromptRu || DOMESTIC_FRAME_FALLBACK, { domestic: true })
    );
    normalizedScene.endFramePromptEn = sanitizeVisiblePromptText(
      sanitizeLegacyPositivePromptArtifacts(normalizedScene.endFramePromptEn || DOMESTIC_FRAME_FALLBACK, { domestic: true })
    );
  }
