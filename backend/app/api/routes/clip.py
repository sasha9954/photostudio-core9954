...
def clip_image(payload: ClipImageIn):
    ...
    try:
        ...
        system_prompt = (
            ...
            "Generate ONE cinematic still frame for a music video scene that looks like a high-budget photograph shot on location. "
            "The result must look like a real professional film frame, never a collage or pasted composite. "
            "REFERENCE RULES: Use all provided references as source of truth. Character references define the SAME person. "
            "Location references define the same world and architecture. Style references define season, weather, palette, atmosphere, lighting mood, and visual language. "
            "Props references define important scene objects. "
            "CHARACTER IDENTITY LOCK: preserve exact facial identity, facial structure, proportions, hairstyle, age, gender, and skin tone. "
            "Do not redesign or beautify/stylize the face beyond identity in references. "
            "Preserve clothing details, logos, and brand marks exactly in shape, color, and placement unless scene text explicitly changes wardrobe. "
            "LOCATION PRESERVATION: keep architecture, street layout, building proportions, landmarks, and perspective so it remains the same place. "
            "You may rebuild in higher cinematic quality, but not a different location. "
            "STYLE APPLICATION: apply style references explicitly and visibly. If style implies winter/snow/cold/icy conditions, show these conditions clearly in environment and lighting. "
            "Do not replace season with a neutral generic look. "
            "PROPS INTEGRATION: do not omit key props. If only one prop exists, treat it as the primary visual object and keep it clearly visible and naturally integrated. "
            "PHYSICAL SCALE CONSISTENCY: keep realistic prop scale relative to the human body and consistent across scenes. "
            "Handheld objects must be realistically liftable and match reference dimensions (never oversized or miniature versions). "
            "HUMAN INTERACTION SCALE: enforce believable ergonomics for grip size, arm reach, lifting posture, and weight distribution. "
            "ENVIRONMENT INTEGRATION: character and props must share the same light direction, intensity, color temperature, reflections, ambient light, and shadows as the environment. "
            "GROUND CONTACT: ensure physical grounding with contact shadows and surface interaction (e.g., compressed snow, footprints, dust displacement) when appropriate. "
            "ATMOSPHERE AND QUALITY: use cinematic lighting, natural depth of field, atmospheric perspective, subtle haze/light scattering, filmic grading, and realistic textures/materials. "
            "Avoid plastic skin, flat lighting, over-sharp synthetic rendering, and collage artifacts. "
            "Scene text may be Russian and visual prompt may be English; use both when available. "
            "Visual prompt defines composition and action, while scene text defines narrative context."
        )
