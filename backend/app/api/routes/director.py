...
DIRECTOR_V2_KNOWLEDGE_VERSION = "director_v2_knowledge_v1"

DIRECTOR_V2_MODE_KNOWLEDGE = {
    "clip": {"mode_goal": "Музыкальный клип с балансом performance и истории.", "audio_role": "Главный таймлайн: ритм, фразы, вокал, энергия, кульминации.", "success_criteria": ["Баланс IA2V/I2V", "Монтаж в ритме аудио", "Понятная арка начала и финала"], "questions": ["Кто поёт?", "Кто главный герой?", "Второй персонаж — кто он в истории?", "Больше performance/lip-sync или больше сюжета?", "Где герой поёт?", "Что показываем между пением?", "Нужны ли воспоминания?", "Какое начало и какой финал?", "Что обязательно сохранить из refs?"], "rules": ["segments — это фразы/смысловые куски, не обязательно сцены", "Можно объединять соседние segment_ids в одну сцену", "lip_sync_candidate = возможно, но не обязательно IA2V", "first_last по умолчанию запрещён без явного запроса"]},
    "story": {"mode_goal": "Визуальная история с понятной драматургией.", "audio_role": "Смысловой нарратив и драматургия.", "success_criteria": ["Читаемая последовательность событий", "Уместный темп", "Ясный конфликт и финал"], "questions": ["Это рассказчик в кадре или voice-over?", "Показываем события напрямую или метафорически?", "Кто главный герой?", "Какой конфликт/поворот?", "Какой финал?", "Нужен ли диалог в кадре?"]},
    "advertisement": {"mode_goal": "Рекламный ролик с фокусом на продукт и CTA.", "audio_role": "Рекламный сценарий: hook/offer/demo/benefit/CTA.", "success_criteria": ["Продукт читается", "Оффер понятен", "CTA явный"], "questions": ["Что рекламируем?", "Кто ЦА?", "Главный offer?", "Что показать крупно?", "Нужна модель или product-only?", "Нужен CTA в конце?", "Что нельзя менять в бренде?"]},
    "kino": {"mode_goal": "Киношная сцена/трейлер с напряжением и развязкой.", "audio_role": "Эмоциональный ритм, реплики, паузы, напряжение.", "success_criteria": ["Кадр-действие-реакция", "Нарастание напряжения", "Выразительная кульминация"], "questions": ["Это сцена, трейлер, диалог, экшен или атмосферный эпизод?", "Кто главный герой?", "Где конфликт?", "Где кульминация?", "Какая камера и настроение?", "Нужны ли реплики в кадре?"]},
    "test": {"mode_goal": "Технический тест pipeline/route/prompt.", "audio_role": "Контроль тайминга и тестовых условий.", "success_criteria": ["Короткий понятный план", "Измеримость", "Контролируемость"], "questions": ["Что тестируем?", "Какой route?", "Проверяем lip-sync, motion, identity, camera или timing?", "Нужна одна сцена или несколько вариантов?"]},
}

DIRECTOR_V2_UNIVERSAL_GRAMMAR = {
    "shot_variety": "Избегай однообразной крупности; разумно чередуй establishing/medium/close-up/detail.",
    "motion_inside_frame": "В сцене должна быть читаемая внутренняя жизнь: герой/камера/среда/эмо-микродвижение.",
    "screen_geography": "Сохраняй понятную экранную географию и направления взглядов (180-degree awareness).",
    "audio_transitions": "Audio timeline — source of truth; J-cut/L-cut допустимы как опция.",
    "tempo_rhythm": "Темп сцены синхронизируй с энергией аудио и narrative beat.",
    "lighting_strategy": "Свет задаёт атмосферу и читаемость без перегруза промта техничностью.",
    "continuity": "Сохраняй identity/wardrobe/world/light family, если смена не является осознанной.",
    "prompt_realism": "Видео-промт должен описывать видимое действие и движение, без копирования внутренних negative-rules.",
}

DIRECTOR_V2_CONNECTED_INPUTS_GUIDE = {
    "character_1": "Потенциальный главный герой/вокалист/рассказчик/protagonist. Если vocal_owner unknown и подключены character_1 + character_2 — обязательно спроси, кто поёт.",
    "character_2": "Второй герой/соперник/любовная линия/друг/антагонист. Если связь с character_1 неясна — спроси про их отношение.",
    "character_3": "Третий персонаж/эпизод/второстепенная линия. Спроси, это важный персонаж или фон.",
    "animal": "Животное может быть главным персонажем, спутником, символом, угрозой, комическим элементом или атмосферной деталью. Если подключено — спроси его сценарную функцию.",
    "group": "Группа может быть толпой, семьёй, бандой, танцорами, хором, командой, массовкой, фанатами. Если подключена — спроси: активная character group или background crowd.",
    "location": "Локация может быть единым миром, памятью, настоящим, product environment, stage/performance world. Спроси: это весь мир ролика или одна зона.",
    "style": "Style reference задаёт visual language, lighting, mood, camera, grade. Спроси: применять на весь ролик или как mood-слой.",
    "props": "Props могут быть сюжетными предметами, брендом/продуктом, символами, деталями кадра. Спроси, какие props обязательны.",
    "video_ref": "video_ref может задавать camera movement, edit rhythm, choreography/motion, lip-sync performance style, lighting/mood, composition. Спроси, какая интерпретация нужна. Не копируй буквально без явного запроса.",
    "text_idea": "Текстовая идея задаёт пользовательский замысел. Её нельзя игнорировать.",
}

def get_director_v2_mode_knowledge(mode: str) -> dict[str, Any]:
    return _safe_dict(DIRECTOR_V2_MODE_KNOWLEDGE.get(str(mode or "").strip().lower()) or DIRECTOR_V2_MODE_KNOWLEDGE.get("clip"))

def get_director_v2_universal_grammar() -> dict[str, Any]:
    return dict(DIRECTOR_V2_UNIVERSAL_GRAMMAR)
...
def _director_v2_chat_system_prompt() -> str:
    return (
        "Ты AI Scenario Director V2 для режимов clip/story/advertisement/kino/test.\n"
        "Ты работаешь после audio_map и не пишешь финальный пайплайн в чате.\n"
        "Используй selected mode, mode_knowledge и universal_directing_grammar из INPUT_JSON.\n"
        "Пиши только на русском. Задавай 2-4 mode-aware вопроса за раз.\n"
        "Сначала уточняй ключевые решения, затем подводи к генерации черновика.\n"
        "audio_map помогает анализом, но не является финальным автором сценария/route plan.\n"
        "Если mode=clip и длительность ~55-60 сек при большом числе сегментов, предлагай объединять фразы в ~10-12 сцен (если пользователь не просит дробный монтаж).\n"
        "first_last не использовать по умолчанию в clip mode.\n"
        "Перед вопросами просмотри connected_inputs_summary.\n"
        "Если подключённая роль есть, но её сценарная функция не ясна — задай уточняющий вопрос.\n"
        "Не задавай вопросы про неподключённые ноды без необходимости.\n"
        "Не игнорируй video_ref, animal, group, если они подключены.\n"
        "video_ref не копируется буквально без запроса; его можно использовать для camera movement/edit rhythm/choreography/lip-sync style/lighting-mood/composition. Если неясно — спроси интерпретацию.\n"
...
{
  "director_contract": {
    "version": "director_v2_knowledge_v1",
    "mode": "clip|story|advertisement|kino|test",
    "format": "9:16|16:9|1:1",
    "content_type": "...",
    "mode_understanding": {"mode_goal":"...","audio_role":"...","success_criteria":[]},
    "story_goal": "...",
    "emotional_arc": "...",
    "visual_world": "...",
    "performance_strategy": "...",
    "audio_interpretation": {"duration_sec":0,"segments_are":"phrases_or_semantic_units","scene_count_strategy":"...","target_scene_count":0,"lip_sync_candidate_policy":"candidate_means_possible_not_required","vocal_owner_role":"...","questions_or_assumptions":[]},
    "route_mix": {
      "ia2v_count": 0,
      "i2v_count": 0,
      "first_last_count": 0,
    ...
    "visual_directing_rules": {"shot_variety":"...","motion_inside_frame":"...","screen_geography":"...","tempo_rhythm":"...","lighting_strategy":"...","continuity_rules":[]},
    "opening_strategy": "...",
    "ending_strategy": "...",
    "reference_usage": {
      "character_1": "...",
    ...
    "connected_input_questions_resolved": {
      "character_1": "resolved|assumption|unresolved|not_connected",
      "character_2": "resolved|assumption|unresolved|not_connected",
      "character_3": "resolved|assumption|unresolved|not_connected",
      "animal": "resolved|assumption|unresolved|not_connected",
      "group": "resolved|assumption|unresolved|not_connected",
      "location": "resolved|assumption|unresolved|not_connected",
      "style": "resolved|assumption|unresolved|not_connected",
      "props": "resolved|assumption|unresolved|not_connected",
      "video_ref": "resolved|assumption|unresolved|not_connected"
    },
    "must_keep": [],
    "must_avoid": [],
    "continuity_rules": [],
    ...
    "downstream_brief": {"core":"...","roles":"...","scenes":"...","prompts":"...","final_video_prompt":"...","final":"..."}
  },
  "draft_plan": [
    {
      "scene_id": "sc_01",
...
- draft_plan должен идти по audio_map.segments в исходном порядке; можно объединять соседние segment_ids.
- segment_id каждого элемента должен быть валидным id из audio_map, а segment_ids (если есть) — только соседние валидные id.
...
def _validate_director_v2_draft(payload: dict[str, Any], parsed: dict[str, Any]) -> list[str]:
    ...
    for i, raw_scene in enumerate(plan):
        ...
        if str(scene.get("route") or "").strip() != route:
...
async def director_v2_chat(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    safe_connected_inputs = _safe_dict(safe_payload.get("connected_inputs"))
    enriched_payload["connected_inputs_guide"] = dict(DIRECTOR_V2_CONNECTED_INPUTS_GUIDE)
    enriched_payload["connected_inputs_summary"] = {
        "has_character_1": bool(safe_connected_inputs.get("ref_character_1")),
        "has_character_2": bool(safe_connected_inputs.get("ref_character_2")),
        "has_character_3": bool(safe_connected_inputs.get("ref_character_3")),
        "has_animal": bool(safe_connected_inputs.get("ref_animal")),
        "has_group": bool(safe_connected_inputs.get("ref_group")),
    ...
    return {"ok": True, "assistant_reply": str(parsed.get("assistant_reply") or "").strip(), "director_memory": _safe_dict(parsed.get("director_memory")), "knowledge_version": DIRECTOR_V2_KNOWLEDGE_VERSION}
...
async def director_v2_draft(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "knowledge_version": DIRECTOR_V2_KNOWLEDGE_VERSION,
    }
