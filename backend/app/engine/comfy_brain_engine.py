...
def build_comfy_planner_prompt(payload: dict[str, Any]) -> str:
    ...
    return (
        ...
        "LIP-SYNC PERFORMANCE RULES (STRICT): performer-first; face/mouth readability is mandatory; eye line toward camera or near-camera is preferred; emotional lyric delivery through face, shoulders, hands, and subtle torso rhythm; allow only gentle head turns, subtle lean, soft sway, phrase-timed hand gestures.\n"
        "LIP-SYNC CAMERA RULES (STRICT): camera must stay slow/controlled/readable; preferred moves are gentle push-in, slight lateral drift, slow eye-level arc; forbidden overhead orbit, top-down spin/rotation, camera roll, spinning around head, aggressive circular camera, fast dolly-out, whip-pan, fast retreating camera.\n"
        "LIP-SYNC BACKGROUND RULE: extras may move softly but stay secondary and non-distracting; no chaotic background dance and no crowd stealing focus.\n"
        "NON-LIP DANCE / MOTION SAFETY RULES: preserve musical energy but keep model-safe movement (smooth groove, controlled weight shift, soft step/pivot/sway, phrase-shaped accents, moderate amplitude/tempo, controlled rotation only); avoid jerky dance, flailing arms, abrupt spin bursts, violent head whipping, stroboscopic pose changes, frantic crowd turbulence.\n"
        "CAMERA ORBIT SAFETY (ALL ROUTES): if orbit is used, it must be a slow horizontal arc around performer at eye/chest/waist level; no overhead path, no top orbit, no head-top camera circle, no drone-like loop, no roll-tilt horizon behavior.\n"
