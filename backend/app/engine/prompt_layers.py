...
def build_ltx_video_canon_block(*, lip_sync: bool) -> str:
    ...
    camera_orientation_lock_rules = [
        ...
        "- CAMERA ORBIT SAFETY: if orbit is used, it must be a slow horizontal arc around the performer",
        "- keep camera near eye/chest/waist level during arc motion",
        "- do not move camera above the subject into top-down orbit and do not spiral over the head",
        "- prefer right-to-left or left-to-right arc around subject plane with stable readability",
    ...
    forbidden_camera_motion_rules = [
        ...
        "- no top orbit, no overhead spin, no head-top camera circle, no drone-like loop over subject, no roll-tilt orbit",
    ]
    if lip_sync:
        lines = [
            "LTX 2.3 LIP-SYNC PERFORMANCE CANON:",
            "- performer-first emotional singing with expressive vocal delivery and emotionally readable eyes",
            "- clear mouth articulation with natural jaw/lips timing, breath detail, and micro-expressions",
            "- face/mouth readability is mandatory; eye line toward camera or near-camera is preferred",
            "- emotional lyric delivery through face, shoulders, hands, and subtle torso rhythm",
            "- allow only gentle head turns, subtle lean, soft sway, and phrase-timed hand gestures",
            "- camera movement must stay slow, controlled, and readable",
            "- preferred camera moves: gentle push-in, slight lateral drift, slow eye-level arc",
            ...
            "- keep framing readable with tight_medium / medium / three_quarter preference",
            "- forbidden: overhead orbit, top-down spin, horizon roll, aggressive circular camera, fast dolly-out, whip-pan, dance-chaos background",
            "- forbidden: no overhead orbit, no top-down rotation, no camera roll, no spinning around head",
            "- forbidden: no aggressive zoom out, no fast retreating camera, no chaotic background dance, no crowd stealing focus",
            "- forbidden: no unreadable mouth, no extreme motion blur, no wide framing drift into generic dance floor shot",
            *camera_orientation_lock_rules,
            *allowed_camera_motion_rules,
            *forbidden_camera_motion_rules,
            *base_physics,
    ...
    lines = [
        ...
        "- movement should feel musical, controlled, and model-safe with physically plausible body mechanics",
        "- controlled elegant motion; use groove, sway, step, pivot, rebound, and pose progression",
        "- use phrase-shaped accents instead of frantic full-body chaos",
        "- prefer camera-led energy over violent body-led motion with moderate movement amplitude",
        "- controlled rotation only; no sudden spin bursts, no violent limb throwing, no rapid hair-whip choreography",
        "- no aggressive torso snapping, no stroboscopic pose changes, no ultra-fast partner motion near hero, no frantic crowd turbulence",
        "- CONTROLLED MOTION SAFETY: smooth readable cinematic motion, grounded body movement, moderate step/sway/turn/weight shift, stable anatomy-safe motion, no jerky movement, no frantic choreography, no violent spins, no high-frequency shaking",
        "- preserve grounded realism and readable continuity without nightlife/club/bar/dancefloor bias",
