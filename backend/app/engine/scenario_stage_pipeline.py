...
from app.engine.scenario_scene_detailer import SCENE_DETAIL_PROMPT_VERSION, build_gemini_scene_detail
...
STAGE_DEPENDENCIES: dict[str, list[str]] = {
    ...
    "scene_detail": ["input_package", "audio_map", "story_core", "role_plan", "scene_plan"],
    "scene_prompts": ["input_package", "audio_map", "story_core", "role_plan", "scene_plan", "scene_detail"],
...
MANUAL_RESET_DOWNSTREAM: dict[str, list[str]] = {
    ...
    "scene_plan": ["scene_detail", "scene_prompts", "final_video_prompt", "finalize"],
    "scene_detail": ["scene_prompts", "final_video_prompt", "finalize"],
...
_IA2V_MEMORY_WORLD_CONFLICT_TOKENS: tuple[str, ...] = (
    "environment cutaway",
    "no main performer",
    "offscreen voiceover",
    "memory/world cutaway",
    "world/cutaway mode",
    "environment-first cutaway",
    "environment-focused still frame",
    "environment-focused motion shot",
)
_PROMPT_SYNC_FIELDS: tuple[str, ...] = (
    "route",
    "photo_prompt",
    "video_prompt",
    ...
    "route_visual_directive",
    "lyric_text_stripped_from_image_prompt",
)
...
def _normalize_director_package_input(package: dict[str, Any]) -> None:
    ...
    diagnostics.setdefault(
        "prompt_validation",
        {
            ...
            "raw_lyrics_in_image_prompt_remaining_count": 0,
            "ia2v_forbidden_clause_remaining_count": 0,
            "ia2v_memory_world_conflict_remaining_count": 0,
        },
    )
    package["diagnostics"] = diagnostics
...
def _apply_director_contract_roles(package: dict[str, Any]) -> None:
    ...
    for row in _safe_list(role_plan.get("scene_casting")):
        ...
        segment_id = str(cast.get("segment_id") or "").strip()
        original_primary = str(cast.get("primary_role") or "").strip().lower()
        ...
        elif route == "i2v" and original_primary == "character_1":
            cast["presence_mode"] = "voiceover"
            cast["presence_weight"] = "support"

        req_refs = required_refs_by_segment.get(segment_id, set())
        forced_primary = ""
        if "character_2" in req_refs:
            forced_primary = "character_2"
        elif "character_1" in req_refs:
            forced_primary = "character_1"
        if forced_primary:
...
def _strip_raw_lyric_anchor(text: str) -> tuple[str, bool]:
    raw = str(text or "")
    had = bool(re.search(r"(?i)lyric/moment anchor:", raw))
    cleaned = re.sub(r"(?i)\s*Lyric/moment anchor:\s*[^.]+\.?", " ", raw)
    cleaned = re.sub(r"\s+", " ", cleaned).strip(" ,.;:")
    return cleaned, had
...
def _clean_director_contract_prompt_row(
    ...
    scene_plan_by_id: dict[str, dict[str, Any]],
) -> dict[str, Any]:
    cleaned_row = dict(_safe_dict(row))
    ...
    seg_id = str(cleaned_row.get("segment_id") or "").strip()
    plan_row = _safe_dict(scene_plan_by_id.get(seg_id))
    route = _to_director_route(cleaned_row.get("route")) or _to_director_route(plan_row.get("route")) or "i2v"
    cleaned_row["route"] = route
    for key in ("photo_prompt", "image_prompt", "first_frame_prompt", "start_image_prompt", "end_image_prompt"):
        cleaned, _ = _strip_raw_lyric_anchor(str(cleaned_row.get(key) or ""))
        cleaned_row[key] = cleaned
    cleaned_row["lyric_text_stripped_from_image_prompt"] = True
    photo = str(cleaned_row.get("photo_prompt") or "")
    video = str(cleaned_row.get("video_prompt") or "")
    if route == "ia2v":
        removed: list[str] = []
        for key in (
            "photo_prompt",
            ...
            key_raw = str(cleaned_row.get(key) or "")
            key_cleaned, removed_key = _strip_ia2v_forbidden_clauses(key_raw)
            cleaned_row[key] = key_cleaned
            removed.extend(removed_key)
        removed = list(dict.fromkeys(removed))
        if removed:
            cleaned_row["forbidden_clause_removed"] = removed
        else:
            cleaned_row.pop("forbidden_clause_removed", None)
        visual_directive = _safe_dict(cleaned_row.get("route_visual_directive"))
        if not visual_directive:
            visual_directive = _safe_dict(plan_row.get("route_visual_directive"))
        zone = _to_contract_location(
            visual_directive.get("location_zone") or cleaned_row.get("location_zone") or plan_row.get("location_zone")
        )
        ...
        photo = str(cleaned_row.get("photo_prompt") or "")
        video = str(cleaned_row.get("video_prompt") or "")
        has_conflict = _ia2v_has_memory_world_conflict(photo, memory_label, memory_zones) or _ia2v_has_memory_world_conflict(
        ...
        safe_emotion = (
            ...
            or str(cleaned_row.get("emotion") or "").strip()
        ...
        if has_conflict:
            cleaned_row["photo_prompt"] = (
                f"{prefix} {identity_clause} {safe_scene_intent}. Emotion: {safe_emotion}. No text or subtitles."
            ).strip()
            rebuilt_video = (
            ...
            if str(cleaned_row.get("positive_video_prompt") or "").strip():
                cleaned_row["positive_video_prompt"] = rebuilt_video
        else:
            cleaned_row["photo_prompt"] = f"{prefix} {photo}".strip()
            cleaned_row["video_prompt"] = f"{prefix} {video}".strip()
            if str(cleaned_row.get("positive_video_prompt") or "").strip():
                cleaned_row["positive_video_prompt"] = f"{prefix} {str(cleaned_row.get('positive_video_prompt') or '')}".strip()
    else:
        visual_directive = _safe_dict(cleaned_row.get("route_visual_directive"))
        if not visual_directive:
            visual_directive = _safe_dict(plan_row.get("route_visual_directive"))
        ...
        for token in (
            r"(?i)\breadable mouth for lip sync\b",
            r"(?i)\bclear expressive lip sync\b",
            r"(?i)\bnatural jaw motion\b",
        ):
            photo = re.sub(token, " ", photo)
            video = re.sub(token, " ", video)
        cleaned_row["photo_prompt"] = f"{prefix} {photo}".strip()
        cleaned_row["video_prompt"] = f"{prefix} {video}".strip()
    cleaned_row["photo_prompt"] = re.sub(r"\s+", " ", str(cleaned_row.get("photo_prompt") or "")).strip(" ,.;")
    cleaned_row["video_prompt"] = re.sub(r"\s+", " ", str(cleaned_row.get("video_prompt") or "")).strip(" ,.;")
    if str(cleaned_row.get("positive_video_prompt") or "").strip():
        cleaned_row["positive_video_prompt"] = re.sub(
            r"\s+",
            " ",
            str(cleaned_row.get("positive_video_prompt") or ""),
        ).strip(" ,.;")
    return cleaned_row
...
def _apply_director_contract_scene_prompts(package: dict[str, Any]) -> None:
    ...
    cleaned_segments = [
        _clean_director_contract_prompt_row(dict(_safe_dict(raw)), contract, scene_plan_by_id)
        for raw in _safe_list(scene_prompts.get("segments"))
    ]
    cleaned_scenes = [
        _clean_director_contract_prompt_row(dict(_safe_dict(raw)), contract, scene_plan_by_id)
        for raw in _safe_list(scene_prompts.get("scenes"))
    ]
    segment_by_id = {
        str(row.get("segment_id") or "").strip(): row
        for row in cleaned_segments
        if str(row.get("segment_id") or "").strip()
    }
    scene_by_id = {
        str(row.get("segment_id") or "").strip(): row
        for row in cleaned_scenes
        if str(row.get("segment_id") or "").strip()
    }
    all_ids = [seg_id for seg_id in list(segment_by_id) + list(scene_by_id) if seg_id]
    ...
    synced_scenes: list[dict[str, Any]] = []
    for seg_id in ordered_ids:
        seg_row = dict(_safe_dict(segment_by_id.get(seg_id)))
        scene_row = dict(_safe_dict(scene_by_id.get(seg_id)))
        canonical = seg_row if seg_row else scene_row
        paired = scene_row if seg_row else {}
        if seg_row and scene_row:
            if any(seg_row.get(field) != scene_row.get(field) for field in _PROMPT_SYNC_FIELDS):
                mismatch_count += 1
            for field in _PROMPT_SYNC_FIELDS:
                scene_row[field] = canonical.get(field)
                seg_row[field] = canonical.get(field)
            synced_segments.append(seg_row)
            synced_scenes.append(scene_row)
        elif seg_row:
            scene_clone = dict(seg_row)
            synced_segments.append(seg_row)
            synced_scenes.append(scene_clone)
        else:
            seg_clone = dict(canonical)
            if paired:
                for field in _PROMPT_SYNC_FIELDS:
                    seg_clone[field] = canonical.get(field)
            synced_segments.append(seg_clone)
            synced_scenes.append(canonical)
    final_rows_for_validation = [*synced_segments, *synced_scenes]
    raw_lyrics_remaining = 0
    ia2v_forbidden_remaining = 0
    ia2v_memory_conflicts_remaining = 0
    _, memory_world = _director_world_role("i2v", contract)
    memory_label = str(memory_world.get("label") or "").strip()
    memory_zones = _director_allowed_zone("i2v", contract)
    for row in final_rows_for_validation:
        if any(
            re.search(r"(?i)lyric/moment anchor:", str(row.get(key) or ""))
            for key in ("image_prompt", "photo_prompt", "first_frame_prompt", "start_image_prompt", "end_image_prompt")
        ):
            raw_lyrics_remaining += 1
        route = _to_director_route(row.get("route")) or "i2v"
        if route != "ia2v":
            continue
    ...
    prompt_validation["final_segments_checked_count"] = len(synced_segments)
    prompt_validation["final_scenes_checked_count"] = len(synced_scenes)
    prompt_validation["raw_lyrics_in_image_prompt_remaining_count"] = raw_lyrics_remaining
    prompt_validation["ia2v_forbidden_clause_remaining_count"] = ia2v_forbidden_remaining
    prompt_validation["ia2v_memory_world_conflict_remaining_count"] = ia2v_memory_conflicts_remaining
    prompt_validation["segments_scenes_prompt_mismatch_count"] = mismatch_count
...
def _apply_director_contract_final_video_prompt(package: dict[str, Any]) -> None:
    ...
    for raw in _safe_list(payload.get("segments")):
        ...
        route_payload = dict(_safe_dict(row.get("route_payload")))
        for key in ("image_prompt", "photo_prompt", "first_frame_prompt", "start_image_prompt", "end_image_prompt"):
            cleaned, had_anchor = _strip_raw_lyric_anchor(str(row.get(key) or ""))
            if had_anchor:
                raw_lyric_removed_count += 1
            row[key] = cleaned
        for key in ("image_prompt", "first_frame_prompt"):
            cleaned, had_anchor = _strip_raw_lyric_anchor(str(route_payload.get(key) or row.get(key) or ""))
            if had_anchor:
                raw_lyric_removed_count += 1
    ...
    diagnostics["final_video_prompt_raw_lyric_anchor_removed_count"] = int(
...
def _has_stage_output(package: dict[str, Any], stage_id: str) -> bool:
    ...
    if stage_id == "scene_detail":
        return _has_non_empty_collection(_safe_dict(output).get("scenes"))
...
def _scene_prompts_dependency_payload_ok(package: dict[str, Any], dependency_stage_id: str) -> bool:
    ...
    if dependency_stage_id == "scene_detail":
        return _has_non_empty_collection(_safe_dict(_safe_dict(pkg).get("scene_detail")).get("scenes"))
...
def _compute_route_budget_for_total(total_scenes: int, creative_config: dict[str, Any]) -> dict[str, int]:
    ...
    if preset_name == "no_first_last_50_50_0":
        return compute_no_first_last_50_50_targets(total_scenes, creative_config.get("extra_scene_policy"))
...
def compute_no_first_last_50_50_targets(scene_count: int, extra_scene_policy: Any = "add_i2v") -> dict[str, int]:
    n = max(0, int(scene_count or 0))
    policy = str(extra_scene_policy or "add_i2v").strip().lower()
    add_ia2v_aliases = {"add_ia2v", "prefer_ia2v", "ia2v", "ia2v_extra"}
    if policy in add_ia2v_aliases:
        i2v = n // 2
        ia2v = n - i2v
    else:
        ia2v = n // 2
...
def _scene_plan_safe_identity_constraints(package: dict[str, Any]) -> dict[str, Any]:
    ...
    connected_summary = _safe_dict(input_pkg.get("connected_context_summary"))
    connected_refs = _safe_dict(connected_summary.get("connectedRefsPresentByRole"))
    ...
    def _roster_blob(role: str) -> str:
        for row in roster_rows:
            row_safe = _safe_dict(row)
            role_id = str(row_safe.get("role_id") or row_safe.get("id") or row_safe.get("entity_id") or "").strip().lower()
            if role_id == role:
...
def _scene_bool(value: Any) -> bool:
...
def _scene_row_vocal_owner_role(row: dict[str, Any]) -> str:
    row_safe = _safe_dict(row)
    candidates = (
        row_safe.get("vocal_owner_role"),
        row_safe.get("speaker_role"),
        _safe_dict(row_safe.get("audio_sensitivity")).get("vocal_owner_role"),
        _safe_dict(row_safe.get("route_visual_directive")).get("vocal_owner_role"),
        _safe_dict(row_safe.get("performance_contract")).get("vocal_owner_role"),
        _safe_dict(row_safe.get("clip_contract")).get("vocal_owner_role"),
    )
...
def _extract_memory_beat_categories(mandatory_beats: list[str]) -> dict[str, list[str]]:
    ...
    for beat_raw in mandatory_beats:
        beat = str(beat_raw or "").strip().lower()
        if not beat:
            continue
        for category, keywords in category_keywords.items():
            matches = [token for token in keywords if token and token in beat]
            if matches:
                extracted.setdefault(category, [])
                for token in matches:
                    if token not in extracted[category]:
                        extracted[category].append(token)
    return extracted
...
def _scene_plan_character_3_unreferenced_episodic_visual_extra(package: dict[str, Any]) -> bool:
    ...
    refs_inventory = _safe_dict(scene_pkg.get("refs_inventory"))
    connected_summary = _safe_dict(input_pkg.get("connected_context_summary"))
    refs_present = _safe_list(_safe_dict(connected_summary.get("refsPresentByRole")).get("character_3"))
    connected_refs_present = _safe_list(_safe_dict(connected_summary.get("connectedRefsPresentByRole")).get("character_3"))
    ...
    if has_visual_ref:
        return False
    char_usage = str(
        _safe_dict(_safe_dict(director_contract.get("reference_usage_contract")).get("character_usage")).get("character_3") or ""
    ).strip().lower()
...
def _build_scene_plan_prompt_package(package: dict[str, Any]) -> dict[str, Any]:
    ...
    if mandatory_memory_beat_categories:
        role_plan["scene_prompt_rules"]["mandatory_memory_beat_categories"] = list(mandatory_memory_beat_categories.keys())
...
def _director_safe_float(value: Any, default: float = 0.0) -> float:
    try:
        if value is None:
            return float(default)
        if isinstance(value, bool):
...
def _build_scene_plan_from_director_draft(package: dict[str, Any]) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    for idx, row_raw in enumerate(draft_plan, start=1):
        ...
        for seg_id in original_segment_ids:
            ...
            t0 = _director_safe_float(
                audio_seg.get("t0", audio_seg.get("start_sec")),
                _director_safe_float(row.get("start_sec"), 0.0),
            )
            t1 = _director_safe_float(
                audio_seg.get("t1", audio_seg.get("end_sec")),
                _director_safe_float(row.get("end_sec"), t0),
            )
...
def _normalize_role_usage_contract(director_contract: dict[str, Any], package: dict[str, Any]) -> dict[str, dict[str, Any]]:
    ...
    input_pkg = _safe_dict(package_safe.get("input"))
    role_usage_source = _safe_dict(contract.get("role_usage_contract"))
    character_contract = _safe_dict(contract.get("character_contract"))
    ref_usage = _safe_dict(_safe_dict(contract.get("reference_usage_contract")).get("character_usage"))
    refs_inventory = _safe_dict(package_safe.get("refs_inventory"))
    connected_summary = _safe_dict(input_pkg.get("connected_context_summary"))
    refs_present = _safe_dict(connected_summary.get("refsPresentByRole"))
    connected_refs = _safe_dict(connected_summary.get("connectedRefsPresentByRole"))

    role_ids: set[str] = set()
    for source_map in (role_usage_source, character_contract, ref_usage, refs_present, connected_refs):
        for key in _safe_dict(source_map).keys():
            token = _canonical_subject_id(key)
            if token and token.startswith("character_"):
                role_ids.add(token)
    for key in _safe_dict(refs_inventory).keys():
        token = str(key or "").strip().lower()
        if token.startswith("ref_character_"):
    ...
    def _normalize_routes(value: Any) -> list[str]:
        if isinstance(value, str):
            value = [chunk.strip() for chunk in re.split(r"[,;/|]", value) if chunk.strip()]
        routes: list[str] = []
        for item in _safe_list(value):
            route = _to_director_route(item)
            if route and route not in routes:
                routes.append(route)
        return routes

    def _read_nullable_bool(*values: Any) -> bool | None:
        for value in values:
            if isinstance(value, bool):
                return value
            token = str(value or "").strip().lower()
    ...
    normalized: dict[str, dict[str, Any]] = {}
    for role_id in sorted(role_ids):
        role_row = _safe_dict(role_usage_source.get(role_id))
        char_row = _safe_dict(character_contract.get(role_id))
        function = str(role_row.get("function") or char_row.get("function") or char_row.get("story_role") or "").strip().lower()
        routes = _normalize_routes(role_row.get("routes") or char_row.get("routes"))
        preferred_route = _to_director_route(role_row.get("preferred_route") or char_row.get("preferred_route"))
        if preferred_route and preferred_route not in routes:
            routes.append(preferred_route)
        ref_usage_token = str(ref_usage.get(role_id) or "").strip().lower()
        reference_required = _read_nullable_bool(
            role_row.get("reference_required"),
            role_row.get("visual_reference_required"),
            char_row.get("reference_required"),
            char_row.get("visual_reference_required"),
        )
        if ref_usage_token == "no_reference_needed":
            reference_required = False
        if _scene_bool(role_row.get("no_reference_needed")) or _scene_bool(char_row.get("no_reference_needed")):
            reference_required = False
        if _scene_bool(role_row.get("reference_required")):
            reference_required = True
        identity_mode = str(role_row.get("identity_mode") or char_row.get("identity_mode") or "").strip().lower()
        story_role = str(char_row.get("story_role") or role_row.get("story_role") or "").strip().lower()
        if not identity_mode and story_role == "episodic":
        ...
        if story_role == "episodic" and reference_required is False and max_scene_count is None:
            max_scene_count = 1
        has_reference_present = any(
            (
                _scene_has_real_ref(_safe_list(refs_present.get(role_id))),
                _scene_has_real_ref(_safe_list(connected_refs.get(role_id))),
                _scene_has_real_ref(_safe_dict(refs_inventory.get(f"ref_{role_id}")).get("refs")),
                _scene_has_real_ref(_safe_dict(refs_inventory.get(f"ref_{role_id}")).get("value")),
                _scene_has_real_ref(_safe_dict(refs_inventory.get(f"ref_{role_id}")).get("count")),
            )
        )
        normalized[role_id] = {
            "role_id": role_id,
            ...
            "identity_mode": identity_mode if identity_mode in {"strict", "loose", "episodic"} else "",
            "max_scene_count": max_scene_count,
            "has_reference_present": bool(has_reference_present),
        }
    return normalized


def _extract_scene_requirements_from_contract(director_contract: dict[str, Any], package: dict[str, Any]) -> list[dict[str, Any]]:
    _ = package
    contract = _safe_dict(director_contract)
    requirement_sources = (
        contract.get("scene_requirements"),
        _safe_dict(contract.get("clip_contract")).get("scene_requirements"),
        _safe_dict(contract.get("scene_contract")).get("scene_requirements"),
    )
    requirements_raw: list[Any] = []
    for source in requirement_sources:
        requirements_raw.extend(_safe_list(source))

    def _normalize_expected_roles(value: Any) -> list[str]:
        roles: list[str] = []
        values = value if isinstance(value, list) else [value]
        for item in values:
            if isinstance(item, dict):
                item = _safe_dict(item).get("role_id") or _safe_dict(item).get("id") or _safe_dict(item).get("role")
            role = _canonical_subject_id(item)
            if role and role not in roles:
                roles.append(role)
        return roles

    def _normalize_expected_role_functions(value: Any) -> list[str]:
        functions: list[str] = []
        values = value if isinstance(value, list) else [value]
        for item in values:
    ...
    for idx, row in enumerate(requirements_raw, start=1):
        row_safe = _safe_dict(row)
        requirement_id = str(row_safe.get("id") or row_safe.get("requirement_id") or f"scene_requirement_{idx:02d}").strip()
        expected_route = _to_director_route(
            row_safe.get("expected_route") or row_safe.get("route") or row_safe.get("required_route")
        )
        expected_roles = _normalize_expected_roles(
            row_safe.get("expected_roles") or row_safe.get("roles") or row_safe.get("required_roles")
        )
        expected_role_functions = _normalize_expected_role_functions(
            row_safe.get("expected_role_functions") or row_safe.get("role_functions")
        )
        expected_world = str(
            row_safe.get("expected_world") or row_safe.get("world") or row_safe.get("required_world") or ""
        ).strip().lower()
        strict = _scene_bool(row_safe.get("strict") or row_safe.get("required"))
        source_text = str(row_safe.get("source_text") or row_safe.get("text") or row_safe.get("description") or "").strip()
        has_structured_expectation = bool(expected_route or expected_roles or expected_role_functions or expected_world)
        normalized.append(
            {
                "id": requirement_id,
    ...
    fallback_sources = (
        _safe_dict(contract.get("memory_contract")).get("mandatory_scenes"),
        _safe_dict(contract.get("clip_contract")).get("mandatory_scenes"),
        _safe_dict(contract.get("scene_contract")).get("mandatory_scenes"),
    )
    fallback_rows: list[str] = []
    for source in fallback_sources:
        fallback_rows.extend(str(item).strip() for item in _safe_list(source) if str(item).strip())
    deduped_rows = list(dict.fromkeys(fallback_rows))
    fallback_requirements: list[dict[str, Any]] = []
    for idx, source_text in enumerate(deduped_rows, start=1):
        expected_route = ""
        lowered = str(source_text).strip().lower()
...
def _build_scene_requirement_context(
    package: dict[str, Any],
    scene_plan: dict[str, Any],
    role_usage_contract: dict[str, dict[str, Any]],
    route_semantics: dict[str, Any],
    ...
    world_alias_map: dict[str, set[str]] = {}
    episodic_aliases_used: list[dict[str, str]] = []

    def add_role_alias(a: Any, b: Any) -> None:
        ra = _canonical_subject_id(a); rb = _canonical_subject_id(b)
        if not ra or not rb:
            return
        role_alias_map.setdefault(ra, set()).add(rb)
        role_alias_map.setdefault(rb, set()).add(ra)

    for k, row in role_usage_contract.items():
        role_id = _canonical_subject_id(k)
        fn = str(_safe_dict(row).get("function") or "").strip().lower()
        if role_id and fn:
            role_function_map.setdefault(role_id, set()).add(fn)

    for src in (_safe_dict(director_contract.get("role_usage_contract")), _safe_dict(director_contract.get("character_contract")), _safe_dict(director_config.get("character_contract"))):
        for key, row in src.items():
            role_id = _canonical_subject_id(key)
            fn = str(_safe_dict(row).get("function") or _safe_dict(row).get("story_role") or "").strip().lower()
            if role_id and fn:
                role_function_map.setdefault(role_id, set()).add(fn)

    for row in _safe_list(_safe_dict(pkg.get("role_plan")).get("roster")):
        role_id = _canonical_subject_id(_safe_dict(row).get("role_id") or _safe_dict(row).get("id"))
        fn = str(_safe_dict(row).get("function") or _safe_dict(row).get("story_role") or "").strip().lower()
        if role_id and fn:
            role_function_map.setdefault(role_id, set()).add(fn)

    for ek, ev in _safe_dict(director_contract.get("episodic_text_characters")).items():
        row = _safe_dict(ev)
        episodic_id = _canonical_subject_id(row.get("role_id") or row.get("id") or ek)
        source_id = _canonical_subject_id(row.get("source_role_id"))
        if episodic_id and source_id:
            add_role_alias(episodic_id, source_id)
            episodic_aliases_used.append({"role_id": episodic_id, "source_role_id": source_id, "function": str(row.get("function") or "").strip().lower()})
            fn = str(row.get("function") or "").strip().lower()
            if fn:
                role_function_map.setdefault(episodic_id, set()).add(fn)
                role_function_map.setdefault(source_id, set()).add(fn)

    def add_world_alias(a: Any, b: Any) -> None:
        wa = str(a or "").strip().lower(); wb = str(b or "").strip().lower()
        if not wa or not wb:
            return
    ...
    return {
        "requirements": _safe_list(requirements),
        "role_alias_map": {k: sorted(v) for k, v in role_alias_map.items()},
        "world_alias_map": {k: sorted(v) for k, v in world_alias_map.items()},
        "role_function_map": {k: sorted(v) for k, v in role_function_map.items()},
        "episodic_aliases_used": episodic_aliases_used,
    }
...
def _role_matches_expected(role_tokens: set[str], expected_role: str, role_alias_map: dict[str, list[str]]) -> bool:
    if expected_role in role_tokens:
        return True
    aliases = set(role_alias_map.get(expected_role, []))
    return bool(aliases & role_tokens)

def _scene_satisfies_requirement(
    ...
    _ = route_semantics
    ctx = _safe_dict(requirement_context)
    row = _safe_dict(scene_row)
    req = _safe_dict(requirement)
    expected_route = _to_director_route(req.get("expected_route"))
    expected_roles = [_canonical_subject_id(value) for value in _safe_list(req.get("expected_roles")) if _canonical_subject_id(value)]
    expected_role_functions = [str(value or "").strip().lower() for value in _safe_list(req.get("expected_role_functions")) if str(value or "").strip()]
    ...
    route = _to_director_route(row.get("route"))
    ...
    if expected_role_functions:
        scene_functions: set[str] = set()
        role_function_map = _safe_dict(ctx.get("role_function_map"))
        for role in role_tokens:
            for token in _safe_list(role_function_map.get(role)):
                scene_functions.add(str(token).strip().lower())
            role_row = _safe_dict(role_usage_contract.get(role))
            token = str(role_row.get("function") or "").strip().lower()
            if token:
                scene_functions.add(token)
        if not all(func in scene_functions for func in expected_role_functions):
            return False
...
def _count_requirement_coverage(
    ...
    for requirement in requirements:
        ...
        if not req_id:
            continue
        matching_scene_indexes: list[int] = []
        ...
        coverage[req_id] = {
            ...
            "match_count": len(matching_scene_indexes),
            "strict": _scene_bool(req.get("strict")),
            "has_structured_expectation": bool(req.get("has_structured_expectation")),
...
def _validate_scene_plan_clip_contract(package: dict[str, Any], scene_plan: dict[str, Any]) -> tuple[bool, list[str]]:
    ...
    requirement_coverage = _count_requirement_coverage(scene_plan, requirements, role_usage_contract, route_semantics, requirement_context)
    missing_requirement_ids = [
        str(req.get("id") or "").strip()
        for req in requirements
        if str(req.get("id") or "").strip()
        and bool(req.get("has_structured_expectation"))
        and (
            _scene_bool(req.get("strict"))
            or req.get("expected_route")
            or _safe_list(req.get("expected_roles"))
            or _safe_list(req.get("expected_role_functions"))
            or str(req.get("expected_world") or "").strip()
        )
        and not bool(_safe_dict(requirement_coverage.get(str(req.get("id") or "").strip())).get("covered"))
    ]
    diagnostics = _safe_dict(package.get("diagnostics"))
    diagnostics["scene_plan_validation_mode"] = "contract_driven" if requirements else "legacy_category_keywords"
    ...
    if not requirements:
        ...
        diagnostics = _safe_dict(package.get("diagnostics"))
        diagnostics["scene_plan_mandatory_memory_beat_categories"] = list(mandatory_categories.keys())
        diagnostics["scene_plan_mandatory_memory_beat_missing_categories"] = list(missing_categories)
        package["diagnostics"] = diagnostics
    ...
    i2v_rows = [row for row in scene_rows if str(_safe_dict(row).get("route") or "").strip().lower() == "i2v"]
    ia2v_rows = [row for row in scene_rows if str(_safe_dict(row).get("route") or "").strip().lower() == "ia2v"]
    role_usage_scene_count: dict[str, int] = {}
    def _scene_row_role_tokens(value: Any) -> list[str]:
        if isinstance(value, dict):
            value_safe = _safe_dict(value)
            token = _canonical_subject_id(
                value_safe.get("role_id") or value_safe.get("id") or value_safe.get("role") or value_safe.get("entity_id")
            )
            return [token] if token else []
        if isinstance(value, list):
            tokens: list[str] = []
            for item in value:
                for token in _scene_row_role_tokens(item):
    ...
    for row in scene_rows:
        ...
        role_tokens.extend(_scene_row_role_tokens(row_safe.get("primary_role")))
        role_tokens.extend(_scene_row_role_tokens(row_safe.get("visual_focus_role")))
        role_tokens.extend(_scene_row_role_tokens(row_safe.get("speaker_role")))
        for field in ("secondary_roles", "refsUsed", "active_roles", "sceneActiveRoles"):
            for token in _scene_row_role_tokens(row_safe.get(field)):
                if token and token not in role_tokens:
                    role_tokens.append(token)
        for role_token in list(dict.fromkeys(role_tokens)):
            role_usage_scene_count[role_token] = int(role_usage_scene_count.get(role_token) or 0) + 1
    unreferenced_episodic_roles: list[str] = []
    for role_id, role_row in role_usage_contract.items():
        row_safe = _safe_dict(role_row)
        reference_required = row_safe.get("reference_required")
        identity_mode = str(row_safe.get("identity_mode") or "").strip().lower()
        max_scene_count = row_safe.get("max_scene_count")
        if reference_required is False and identity_mode in {"loose", "episodic"} and isinstance(max_scene_count, int) and max_scene_count > 0:
            unreferenced_episodic_roles.append(role_id)
            if int(role_usage_scene_count.get(role_id) or 0) > int(max_scene_count):
                errors.append(f"unreferenced_episodic_visual_extra_used_too_many_scenes:{role_id}")
...
def _seed_missing_scene_requirements_from_contract(
    ...
    requirement_map = {
        ...
        for req in _safe_list(requirements)
    ...
    for requirement_id in [str(v or "").strip() for v in missing_requirement_ids if str(v or "").strip()]:
        ...
        source_text = str(req.get("source_text") or "").strip()
        if source_text and str(row.get("requirement_source_text") or "").strip() != source_text:
            row["requirement_source_text"] = source_text
            changed.add("requirement_source_text")
        purpose = str(req.get("purpose") or "").strip()
        if purpose and str(row.get("requirement_purpose") or "").strip() != purpose:
            row["requirement_purpose"] = purpose
            changed.add("requirement_purpose")
        ...
        primary_role = _canonical_subject_id(row.get("primary_role"))
        if expected_roles and not primary_role:
...
def _build_scene_plan_retry_feedback(validation_error: str, error_code: str, scene_diag: dict[str, Any]) -> str:
    ...
    if ve.startswith("scene_requirement_missing:"):
        ...
        for row in requirements:
            row_safe = _safe_dict(row)
            if str(row_safe.get("id") or "").strip() == requirement_id:
                requirement = row_safe
                break
...
def _validate_scene_plan_route_budget(
    *,
    ...
    odd_extra_policy = str(creative_config.get("extra_scene_policy") or "add_i2v").strip().lower()
    mode = str(creative_config.get("route_mix_mode") or "auto").strip().lower() or "auto"
    preset_name = str(creative_config.get("route_strategy_preset") or "").strip().lower()
    original_targets = {
    ...
    details = {
        ...
        "scene_plan_route_budget_odd_extra_policy": odd_extra_policy,
        "scene_plan_route_budget_expected_from_extra_policy": dict(target_budget),
    }
    ...
    *,
    ...
    def _memory_subject_role_candidate() -> str:
        for role_id, role_row in role_usage_contract.items():
            ...
            row_safe = _safe_dict(role_row)
            funcs: set[str] = set()
            single_fn = str(row_safe.get("function") or "").strip().lower()
            if single_fn:
                funcs.add(single_fn)
            for value in _safe_list(row_safe.get("functions")):
                fn_token = str(value).strip().lower()
                if fn_token:
                    funcs.add(fn_token)
    ...
    def _role_is_i2v_compatible(role_token: str) -> bool:
        role_row = _safe_dict(role_usage_contract.get(role_token))
        if not role_row:
            return False
        routes = {str(v).strip().lower() for v in _safe_list(role_row.get("routes")) if str(v).strip()}
        if "i2v" not in routes:
            return False
        funcs: set[str] = set()
        single_fn = str(role_row.get("function") or "").strip().lower()
        if single_fn:
            funcs.add(single_fn)
        for value in _safe_list(role_row.get("functions")):
            fn_token = str(value).strip().lower()
            if fn_token:
                funcs.add(fn_token)
        if funcs.intersection({"performer", "singer", "speaker"}):
            return False
        return True
        ...
        for idx, row in enumerate(rows, start=1):
            ...
            if strict_hard_contract and hard_locked_route == "i2v":
                ...
                if _clean_i2v_lipsync_fields(row):
                    changed = True
                current_primary_role = _canonical_subject_id(row.get("primary_role"))
                if not _role_is_i2v_compatible(current_primary_role):
                    ...
                    if memory_role:
                        ...
                        role_functions = [str(v).strip() for v in _safe_list(row.get("role_functions")) if str(v).strip()]
                        role_functions_lower = {v.lower() for v in role_functions}
                        if "memory_subject" not in role_functions_lower:
                            role_functions.append("memory_subject")
                        row["role_functions"] = role_functions
            ...
            if resolved_primary == "world":
                ...
                if speaker_role:
                    row["speaker_role"] = ""
                    changed = True
                if _scene_bool(row.get("lip_sync_allowed")):
                    row["lip_sync_allowed"] = False
                    changed = True
                if _scene_bool(row.get("mouth_visible_required")):
...
def _run_role_plan_stage(package: dict[str, Any]) -> dict[str, Any]:
    ...
    diagnostics["active_role_planner_code_path"] = str(role_diag.get("active_role_planner_code_path") or diagnostics.get("active_role_planner_code_path") or "scenario_role_planner_v2")
    diagnostics["director_draft_intents_count"] = int(role_diag.get("director_draft_intents_count") or diagnostics.get("director_draft_intents_count") or 0)
    diagnostics["director_role_overrides_applied_count"] = int(role_diag.get("director_role_overrides_applied_count") or diagnostics.get("director_role_overrides_applied_count") or 0)
    diagnostics["director_blocking_primary_mismatches"] = _safe_list(role_diag.get("director_blocking_primary_mismatches") or diagnostics.get("director_blocking_primary_mismatches"))
    diagnostics["director_core_mismatches_overridden_by_draft"] = _safe_list(role_diag.get("director_core_mismatches_overridden_by_draft") or diagnostics.get("director_core_mismatches_overridden_by_draft"))
...
def _run_scene_plan_stage(package: dict[str, Any]) -> dict[str, Any]:
    ...
    if strict_preset_name == "no_first_last_50_50_0":
        backend_hard_route_map = build_no_first_last_50_50_hard_route_map(scene_plan_prompt_package)
        diagnostics["scene_plan_hard_route_map_uses_sanitized_package"] = True
    ...
    if not clip_contract_ok:
        ...
        clip_feedback = (
            ...
            "route budget, and satisfy contract scene requirements. "
            f"Fix errors: {'; '.join(clip_contract_errors[:12])}."
        )
        if contract_requirement_feedback:
            clip_feedback = f"{clip_feedback}\n{contract_requirement_feedback}"
        retry_result = build_gemini_scene_plan(
            api_key=gemini_api_key,
            package=scene_plan_prompt_package,
        ...
        if retry_scene_plan:
            ...
            if clip_retry_ok:
                ...
                merged_director_contract = _build_merged_director_contract_for_scene_validation(package)
                role_usage_contract = _normalize_role_usage_contract(
                    merged_director_contract,
                    package,
                )
                seed_requirements = _extract_scene_requirements_from_contract(merged_director_contract, package)
                requirement_context = _build_scene_requirement_context(
                    package,
                    retry_scene_plan,
                    role_usage_contract,
                    _safe_dict(merged_director_contract.get("route_semantics")),
                    seed_requirements,
                )
                ...
                seeded_scene_plan, seed_sync_meta = _sync_scene_plan_storyboard_mirror(seeded_scene_plan)
                diagnostics = _safe_dict(package.get("diagnostics"))
                diagnostics.update(_safe_dict(seed_diag))
                diagnostics["scene_plan_requirement_contract_seed_sync"] = _safe_dict(seed_sync_meta)
                package["diagnostics"] = diagnostics
    ...
    resolved_vocal_owner = str(
        diagnostics.get("scene_plan_vocal_owner_role_forced")
        or scene_diag.get("vocal_owner_role")
        or "unknown"
    ).strip().lower()
    diagnostics["scenes_vocal_owner_role_used"] = resolved_vocal_owner or "unknown"
    if not route_budget_ok:
...
def _run_scene_detail_stage(package: dict[str, Any]) -> dict[str, Any]:
    diagnostics = _safe_dict(package.get("diagnostics"))
    diagnostics["scene_detail_backend"] = "gemini"
    diagnostics["scene_detail_prompt_version"] = SCENE_DETAIL_PROMPT_VERSION
    package["diagnostics"] = diagnostics
...
def _run_scene_prompts_stage(package: dict[str, Any]) -> dict[str, Any]:
    ...
    scene_detail_rows = _safe_list(_safe_dict(package.get("scene_detail")).get("scenes"))
    prompts_package = package
    if scene_detail_rows:
        prompts_package = deepcopy(package)
        prompts_package["scene_plan"] = {**_safe_dict(prompts_package.get("scene_plan")), "scenes": scene_detail_rows, "storyboard": scene_detail_rows}
...
def run_stage(stage_id: str, package: dict[str, Any], payload: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    try:
        ...
        elif stage_id == "scene_detail":
            pkg = _run_scene_detail_stage(pkg)
